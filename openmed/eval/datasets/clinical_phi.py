...
CLINICAL_PHI_MANIFEST_REF = (
    "openmed.eval.datasets:load_clinical_phi_manifest@"
    f"{CLINICAL_PHI_MANIFEST_ID}"
)
...
class ClinicalPHIDatasetManifest:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "label_groups": {
                name: list(values)
                for name, values in sorted(self.label_groups.items())
            },
    ...
    for source in manifest.sources:
        ...
        if source.access == "credentialed_eval_only":
            if not source.requires_credentials or not source.eval_only:
                raise ValueError("DUA sources must be credentialed eval-only references")
